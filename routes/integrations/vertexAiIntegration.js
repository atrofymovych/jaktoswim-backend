...
const AI_CONFIG = {
  project: 'aqueous-freedom-436309-m4',
  location: process.env.VERTEX_AI_LOCATION || 'us-central1',
  defaultModel: process.env.GEMINI_MODEL || 'gemini-2.0-flash-001',
  defaultSystemPrompt: process.env.AI_SYSTEM_PROMPT || '',
};

const vertexAI = new VertexAI({ project: AI_CONFIG.project, location: AI_CONFIG.location });
...
async function callVertexChat({ orgId, model, systemPrompt, history = [], message }) {
  const startTime = Date.now();

  try {
    const vertexAI = new VertexAI({ project: AI_CONFIG.project, location: AI_CONFIG.location });
    const gen = vertexAI.getGenerativeModel({ model: model || AI_CONFIG.defaultModel });
    ...
    if (systemPrompt && String(systemPrompt).trim().length) {
      req.systemInstruction = { parts: [{ text: String(systemPrompt) }] };
    }

    const res = await gen.generateContent(req);
    const response = res?.response?.candidates?.[0]?.content?.parts?.[0]?.text || '';

    // Track AI response time safely
    try {
      const duration = (Date.now() - startTime) / 1000;
      trackAiResponseTime(orgId, 'vertex', model || AI_CONFIG.defaultModel, duration);
    } catch (error) {
...
router.post('/sessions/:sessionId/ask', async (req, res) => {
  try {
    ...
    setImmediate(async () => {
      try {
        const answer = await callVertexChat({
          model: model || AI_CONFIG.defaultModel,
          systemPrompt: typeof system_prompt === 'string' ? system_prompt : AI_CONFIG.defaultSystemPrompt,
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        const result = await callVertexChat({
          model: model || AI_CONFIG.defaultModel,
          systemPrompt: typeof system_prompt === 'string' ? system_prompt : sys,
          history: hist,
          message: task,
        });
